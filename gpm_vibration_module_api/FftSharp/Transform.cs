...
namespace FftSharp
{
    internal static class Transform
    {
        ...
        /// </summary>
        /// <param name="input">real input</param>
        public static double[] FFTpower(double[] input,bool IsdBUnit=true)
        {
            if (!IsPowerOfTwo(input.Length))
                throw new ArgumentException("Input length must be an even power of 2");
        ...
        public static List<double> FFTpower(List<double> input, bool IsdBUnit = true)
        {
            return FFTpower(input.ToArray(), IsdBUnit).ToList();
        }
