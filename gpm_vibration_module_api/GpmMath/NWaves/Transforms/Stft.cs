...
namespace NWaves.Transforms
{
    ...
    public class Stft
    {
        ...
        public List<Tuple<float[], float[]>> Direct(float[] input)
        {
            // pre-allocate memory:

            var len = (input.Length - _windowSize) / _hopSize + 1;

            var stft = new List<Tuple<float[], float[]>>(len);
            ...
            var lre = stft.Last().Item1;
            var lim = stft.Last().Item2;
        ...
        public float[] Inverse(List< Tuple<float[], float[]>> stft, bool perfectReconstruction = true)
        {
